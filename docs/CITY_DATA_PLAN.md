...
**No duplicate legality logic.** The orchestrator never independently computes duration, applicability, or a status — it calls `evaluateParkingLegality(rules, interval)` unchanged, once per candidate. `evaluateParkingLegality` remains the single legality authority in this codebase.

**Verification.** `scripts/verify-orchestration.ts` (`pnpm verify:orchestration`) is a small, zero-dependency script — not a new test framework — using fake injected `fetchNearbySpots`/`fetchRulesForCandidate` functions. It checks 10 cases: no candidates → `[]`; a candidate with no rules → `UNKNOWN`; a confirmed all-day safe `TIME_LIMIT` → `LEGAL`; a confirmed exceeded `TIME_LIMIT` → `ILLEGAL`; an unresolved `TIME_LIMIT` → `UNKNOWN`; a candidate simulating no city-rule association → `UNKNOWN`; candidate ordering preserved (nearest-first, independent of input order); one candidate's simulated rule-lookup failure isolated from another candidate's successful evaluation; three candidates evaluated independently to three different verdicts; and a simulated candidate-discovery failure propagating (not swallowed).
...
**PRIMARY FINDING — `permit_area` is currently always null in practice, and this is a verified mapping gap, not a data-sparsity issue.** `ingestRegulations()` reads `permit_area` from Socrata keys `["permitarea", "permit_area"]`. Neither key exists anywhere in this dataset's real rows — the actual field is named `rpparea1` (with `rpparea2`/`rpparea3` present on a minority of rows for blocks with more than one permit area). Profiling `rpparea1` directly found it non-null on 2,500 of 3,000 rows (83%) — real, common data — but it is not reaching `city_parking_blocks.permit_area` today because of this key-name mismatch. **This milestone did not fix it** (out of scope — profiling only); it is reported here as a concrete, verified fact for a future, separate ingestion-mapping fix to address.
...
- `permit_area` cannot be used for anything yet — not because the underlying data is bad (83% of rows have a real `rpparea1` value), but because it is not currently reaching `ParkingRule.permitArea` at all, per the PRIMARY FINDING above.
