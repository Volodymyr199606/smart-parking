...
**Current curb store selection/recovery milestone:** [AWS S3 recommendation and local proof](./CITY_CURB_ARTIFACT_STORE.md). **PROVIDER SELECTED; LOCAL STORE TESTED; RECOVERY CONTRACT TESTED; CLOUD RETENTION NOT YET VERIFIED.** The retained 18,355-row archive passed full recovery through a separate local object store, preserving exact package/dataset/geometry hashes and 100% eligibility. AWS code remains a transport-injected skeleton with no cloud SDK or credentials. User-approved provider provisioning/retention policy, SDK transport, independent verifier credentials/DB integration and reviewed production rollout remain prerequisites. No cloud upload, database operation or runtime/coverage change occurred; CITY remains INCOMPLETE.
