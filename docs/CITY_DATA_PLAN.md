...
**Curb storage migration implementation V1 — current status.** **DESIGN COMPLETE; MIGRATION IMPLEMENTED; POSTGRES EXECUTION TEST PENDING; PRODUCTION ROLLOUT BLOCKED.** [00012_city_parking_curb_storage.sql](../supabase/migrations/00012_city_parking_curb_storage.sql) promotes the reviewed publication SQL unchanged into one additive, empty-data migration. It declares snapshots, immutable content versions, staging-only membership and a per-source current publication pointer, with trusted validation/publication/failure functions, source-first locks, RESTRICT FKs, DELETE/TRUNCATE guards and server-only access. The verifier now checks the actual migration and its agreement with the contract. No local PostgreSQL/Docker was found; static checks do not prove engine behavior or production readiness. Source registration, independent verifier authentication, live capture and immutable artifact retention remain later work. No migration was applied, database contacted, source ingested, association populated or runtime changed. CITY remains **INCOMPLETE**. This implementation status supersedes migration-file deferral in the earlier milestone summaries above; their historical profiling results remain unchanged.
