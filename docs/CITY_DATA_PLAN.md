...
**Current curb AWS adapter milestone:** [S3 SDK adapter and cloud acceptance harness V1](./CITY_CURB_ARTIFACT_STORE.md). **S3 ADAPTER IMPLEMENTED; S3 MOCK-TESTED; AWS RESOURCES NOT PROVISIONED; NO CLOUD UPLOAD PERFORMED; CLOUD ACCEPTANCE PENDING.** Conditional writes, pinned checksum recovery, COMPLIANCE/bucket checks, IAM templates and an offline-by-default command are implemented. The first executable cloud path permits only a fixed two-row synthetic fixture; real-archive execution is blocked. The previous 18,355-row local recovery remains valid. This task did not inspect/provision AWS or credential values. Manual bucket/role setup, authorized synthetic acceptance, independent verifier recovery/protected receipts, subsequent real-upload approval and DB integration remain gates. No cloud service, database or runtime/coverage operation occurred; CITY remains INCOMPLETE.

**Current curb artifact retention milestone:** [Artifact retention and full offline replay V1](./CITY_CURB_ARTIFACT_RETENTION.md) resolves operational full-archive verification locally: retained source bytes independently reconstruct all 18,355 rows, expected hashes and zero ineligible rows, with deterministic repeat reports and corruption tests. Package checksums cover metadata/count responses as well as pages; archive-content seals deliberately exclude DB UUIDs. True durable storage is still unconfigured. Independent verifier authentication/DB comparison, retention/recovery proof and reviewed source-registration/production rollout remain gates. No database, source network, migration or runtime work occurred; CITY remains INCOMPLETE.

**Current curb V2 publication compatibility:** [00014 design and tests](./CITY_CURB_PUBLICATION_CONTRACT.md#current-v2-validation-and-publication-compatibility-v1) closes the local V2 validation/publication compatibility gap through independent immutable attestations, versioned membership seals and guarded pointer advancement. Capture/eligibility/staging do not imply verification or publication. V1 history and 00012/00013 files are preserved; source hostname upgrades keep the same logical dataset and exact historical capture URLs. Fresh local synthetic tests cover role separation, stale evidence, retries and concurrency. No production access, real-source backfill, runtime or coverage change occurred. Independent verifier provisioning, durable artifacts and reviewed production rollout remain prerequisites; CITY remains INCOMPLETE.

**Current curb V2 eligibility/staging:** [Full contract and evidence](./CITY_CURB_INGESTION_BOUNDARY.md#current-v2-ingest-eligibility-and-staging-rpc-v1). **18,355/18,355** retained curb rows are DB-ingest-eligible (zero rejected); pure eligibility tests **33**, fresh local RPC tests **71**. Forward migration **00013** adds restricted TEXT staging/read RPCs, DB numeric/readback guards and idempotent immutable membership. 00012, the capture codec and runtime are unchanged. Capture validity → DB eligibility → staging → independent verification → publication remain separate gates. V2 verification/publication compatibility, durable artifacts, verifier authentication, source registration and reviewed production rollout remain outstanding. Production was never contacted; CITY remains INCOMPLETE. Earlier boundary statements below are historical.
...
**Curb storage migration implementation V1 — current status.** **STATIC VERIFIED; POSTGRES COMPILED; LIFECYCLE TESTED; IMMUTABILITY TESTED; ROLLBACK TESTED; CONCURRENCY TESTED; RLS/PRIVILEGES TESTED — LOCALLY. PRODUCTION NOT APPLIED; PRODUCTION ROLLOUT BLOCKED.** [00012_city_parking_curb_storage.sql](../supabase/migrations/00012_city_parking_curb_storage.sql) declares snapshots, immutable content versions, staging-only membership and a per-source current publication pointer with guarded lifecycle functions and server-only access. The prior ownership correction retains executor ownership (confirmed locally as `postgres`) and verifier/client separation. Migrations 00001-00012 applied successfully to fresh local Supabase PostgreSQL 17.6. The subsequent synthetic behavior harness passed 132 checks, including actual role operations, shadow resistance, rollback and two-publisher lock serialization with exactly one advancement. No new migration defect or migration edit/reset was needed. Typecheck and 41 static checks passed. Exact commands, retained-fixture behavior and evidence are in the [publication contract](./CITY_CURB_PUBLICATION_CONTRACT.md#7-migration-structure-and-verification-boundary). Real source registration, independent verifier authentication, live capture and immutable artifact retention remain blockers; local placeholders do not satisfy them. No production connection, source ingestion, association population or runtime change occurred. CITY remains **INCOMPLETE**. This status supersedes earlier milestone deferral statements; historical profiling results remain unchanged.

**Live curb snapshot capture validation V1 (2026-09-22).** **LIVE QUERY VERIFIED; DOUBLE CAPTURE NOT VERIFIED; METADATA AVAILABLE on `data.sf.gov`; PRODUCTION PUBLICATION STILL BLOCKED.** The legacy hostname returned 403 for ordered/count queries and redirected metadata/simple rows to `data.sf.gov`. The current public DataSF host accepted the unchanged five-row ordered query, metadata and count (18,355). Two independent bounded attempts each retained one byte-identical 1,000-row page, with 1,000 distinct nonblank ascending IDs and zero retries, before the unchanged `curb-jcs-v1` decimal precision guard rejected real coordinate tokens. Both attempts are INVALID. Checksums and partial before-schema equality passed; full pagination, full identity comparison and feature/dataset digest reproducibility remain unverified. The capture tool now pins the current host, retains offline compatibility with legacy archives, supports one independent attempt and pauses 15 seconds between successful captures in comparison mode. Typecheck and 34 offline canonicalization checks passed. See the [snapshot contract](./CITY_CURB_SNAPSHOT_CONTRACT.md#10-verification-and-readiness) for endpoint diagnostics, exact precision examples and retained local evidence. A lossless numeric contract decision, old-host DB guard compatibility, complete live validation, durable retention, independent verifier authentication and real source registration still block rollout. No database access/write, migration apply, ingest, runtime or coverage change occurred; CITY remains **INCOMPLETE**.

**City curb lossless numeric canonicalization V2 — current capture status.** **LIVE ORDERED QUERY VERIFIED; LOSSLESS V2 SINGLE CAPTURE VERIFIED; LOSSLESS V2 DOUBLE CAPTURE VERIFIED; PRODUCTION PUBLICATION BLOCKED.** The unchanged current codec is `curb-decimal-v2` with `curb-snapshot-v2` manifests and `/v2` logical hash domains. Frozen decimal wrappers preserve exact numeric source values; V1 hashes/interval/SQL fixtures remain separately reproducible. Following the successful single-capture milestone, two new independent public DataSF captures on 2026-09-22 each fetched 18,355 unique rows over 19 pages with zero duplicates, missing IDs or retries. A 15-second pause followed A's successful reconstruction. Both are CONSISTENT; source count, metadata and schema observations matched. IDs only in A/B, changed-content IDs and changed-geometry IDs are all zero; all 18,355 feature hash sets and all raw page bytes match. Reconstruction from retained artifacts passed for both without network access. Manifest differences are limited to capture/retrieval timestamps and HTTP Date headers. **Live double-capture reproducibility is RESOLVED for the observed source state**, without claiming transaction isolation or CITY completeness. Typecheck, 100 V2 plus 34 preserved V1 checks, 41 publication static checks and 38 interval checks passed again. See the [double-capture evidence](./CITY_CURB_SNAPSHOT_CONTRACT.md#12-live-double-capture-validation-v2) for exact hashes, times and local artifact paths. Production still requires a precision-safe database/client ingestion boundary, V2/endpoint guard compatibility, durable immutable retention, independent verifier authentication and source registration/rollout procedure. This continuation changed only three documents and preserved the earlier uncommitted implementation work. No database connection/write, migration apply, ingestion, runtime or coverage change occurred; CITY remains INCOMPLETE.
