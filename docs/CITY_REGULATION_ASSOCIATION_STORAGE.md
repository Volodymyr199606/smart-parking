...
**Curb publication follow-up:** [Publication + Immutability Guards V1](./CITY_CURB_PUBLICATION_CONTRACT.md) defines append-only curb versions, validation-frozen membership and terminal PUBLISHED snapshots with a separate current pointer. Old PUBLISHED snapshots remain reproducible even when superseded. An association run must still validate membership in its exact target snapshot and current-release eligibility separately. This target-storage guard draft does not implement the association run/review guards below. PostgreSQL execution testing and operational rollout gates remain outstanding; no migration or association population was added.

**Interval contract follow-up:** [Curb Interval Measurement V1](./CITY_CURB_INTERVAL_CONTRACT.md) specifies `sf-curb-planar-um-v1`, nine-decimal target fractions, all-segment projection with explicit ties, and conditional continuous-extent evidence. Its proposed target `numeric(10,9)` fields/model/component invariants supersede the older target-fraction precision in the conditional SQL draft below; no SQL is created or applied in this milestone. Source-regulation interval conventions remain separate. Selected duplicates/overlaps for one regulation/version require rejection or review, adjacent intervals remain separate, and stacked regulations are allowed. Measurement validity is not association approval.
...
The target curb canonicalization and interval measurement contracts are now defined and offline-tested. Open prerequisites: persisted curb-version/snapshot identity and source registration; live snapshot validation and operational artifact retention; source geometry retention/measurement linked to regulation versions; positional-error/cross-engine calibration and independent side/extent labels; reviewed-set publication/immutability guards and authenticated operator policy; source-change invalidation; and the missing parking-location-to-curb link. The SQL and TypeScript proposals intentionally remain documentation until these dependencies are resolved.
