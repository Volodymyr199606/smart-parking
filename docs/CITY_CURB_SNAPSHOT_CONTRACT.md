...
**Current artifact follow-up:** [Durable artifact contract and full archive verifier V1](./CITY_CURB_ARTIFACT_RETENTION.md) preserves this capture layout and bytes, adds a complete package checksum and an offline verifier that distrusts manifest summaries. Two full retained-archive replays reconstructed 18,355 rows with matching digests and 18,355 eligible / zero ineligible. Local offline verification is resolved; a real immutable storage provider, independent credentials/DB verification integration and production rollout remain outstanding. Earlier operational-verifier blocker statements below describe prior milestones. No migration or runtime change occurred.

**Current validation/publication follow-up:** [V2 compatibility migration 00014](./CITY_CURB_PUBLICATION_CONTRACT.md#current-v2-validation-and-publication-compatibility-v1) accepts the verified `curb-decimal-v2` / `curb-snapshot-v2` / `data.sf.gov` contract after independent verifier evidence, complete eligibility counts and a source/snapshot-bound membership seal. Capture bytes, V2 hash domains and V1 reproduction are unchanged; actual historical V1 is `curb-jcs-v1`, not `curb-decimal-v1`. Eligibility evidence is recorded separately from the capture manifest. Attestation and validation commit atomically; publication uses immutable metadata/seals rather than parsing source numerics. The real 18,355 rows were not staged/published; all new DB evidence is disposable synthetic local testing. Production remains unapplied.

**Current DB eligibility/staging follow-up:** [V2 storage domain and 00013 local execution](./CITY_CURB_INGESTION_BOUNDARY.md#current-v2-ingest-eligibility-and-staging-rpc-v1) distinguishes capture-valid evidence from DB-ingest-eligible features. All **18,355** retained rows passed; the observed numeric maxima are 19 token characters, 17 significant digits and 15 fractional digits. The capture contract and artifacts remain unchanged. New pure eligibility checks and server-only TEXT staging/read RPCs passed 33 offline / 71 local DB tests. Ineligible future captures remain valid historical evidence where capture rules permit; they must not be rounded or silently filtered into a complete publication. Manifest `summary.locally_eligible` retains its original capture meaning. V2 staging is locally verified; independent V2 verification/publication and production rollout remain blocked. No production connection occurred.

**Database boundary follow-up: LOSSLESS CAPTURE VERIFIED; LOSSLESS DB TEXT BOUNDARY VERIFIED locally; ORDINARY JS OBJECT PATH UNSAFE; PRODUCTION INGESTION NOT YET ENABLED.** The [ingestion boundary proof](./CITY_CURB_INGESTION_BOUNDARY.md) passed 44 real Supabase JS/PostgREST/PostgreSQL checks, including observed coordinates and five complete feature hash round trips. TEXT preserves numeric tokens within the documented JSONB/readback domain; PostgreSQL range and expanded-token parser limits require fail-closed eligibility checks. Migration 00012's V1/legacy-host guards remain unchanged and block V2 production storage. Earlier “no database” and review-only statements below describe their original milestones; this follow-up used only disposable local test objects and cleaned them up.
