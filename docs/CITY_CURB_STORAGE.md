...
**Current store/recovery follow-up:** [Provider selection and recovery proof V1](./CITY_CURB_ARTIFACT_STORE.md) selects AWS S3 as primary and Backblaze B2 as a fallback requiring provider-specific acceptance. Object keys bind the full artifact package, avoiding collisions between different captures with equal logical dataset hashes. Local conditional writes and exact-byte recovery passed; existing URI/digest fields remain sufficient, with pinned object versions in independently retained receipts. No migration, production/cloud access or real DB staging occurred. Cloud retention remains unverified.

**Current artifact follow-up:** [Durable artifact contract and full archive verifier V1](./CITY_CURB_ARTIFACT_RETENTION.md) defines immutable package identity, provider-neutral references and retention/recovery requirements. Existing 00012/00014 fields are sufficient; no migration is required. Complete lossless offline verification of the retained 18,355 rows passes twice, including full DB eligibility. Local files are not yet durable retained artifacts, and offline reports are not DB attestations. No storage provider, production connection or runtime change was introduced.

**Current V2 validation/publication follow-up:** [00014 contract and local proof](./CITY_CURB_PUBLICATION_CONTRACT.md#current-v2-validation-and-publication-compatibility-v1) adds one immutable attestation table and version-specific sealing/validation/publication. 00012/00013 files remain unchanged. Independent artifact replay and eligibility evidence must bind the complete frozen membership; staging cannot self-attest. The logical source survives the reviewed legacy-to-current endpoint upgrade without rewriting snapshot URLs. CAPTURE, ELIGIBILITY, STAGING, ATTESTATION, VALIDATION and PUBLICATION remain separate gates. Full fresh migration-chain and synthetic role/lifecycle tests are local only; production rollout/credentials/durable artifact operations remain outstanding. CITY remains INCOMPLETE.

**Current V2 staging milestone:** [Eligibility contract and local RPC evidence](./CITY_CURB_INGESTION_BOUNDARY.md#current-v2-ingest-eligibility-and-staging-rpc-v1): all **18,355** retained rows are DB-ingest-eligible, with zero rejections. The pure checker passed 33 tests; the real local staging/read RPC harness passed 71. New forward migration **00013** supports V2 STAGING via a server-only TEXT boundary with SQL eligibility guards, immutable reuse and idempotent membership. **00012 remains unchanged.** Capture validity, DB eligibility, staging, independent verification and publication are separate states. V2 publication remains blocked by the unchanged V1 attestation/completeness gate; production was never contacted. Only fresh disposable local synthetic data was written and removed. CITY remains INCOMPLETE. Older “future RPC/eligibility” statements below describe prior milestones.

**Lossless database boundary follow-up:** [Local TEXT RPC proof and proposed ingestion interface](./CITY_CURB_INGESTION_BOUNDARY.md): **LOSSLESS CAPTURE VERIFIED; LOSSLESS DB TEXT BOUNDARY VERIFIED within the tested/bounded domain; ORDINARY JS OBJECT PATH UNSAFE; PRODUCTION INGESTION NOT YET ENABLED.** All 44 local checks passed, including exact coordinate storage, Supabase string preservation and five complete feature hash round trips. JSONB column types are suitable, but 00012 still pins V1 contracts and legacy source endpoints. Its SQL was neither edited nor applied. Explicit JSONB/readback eligibility, forward V2 compatibility work and authenticated artifact verification remain required. Only disposable local proof objects were written and removed; existing curb tables were untouched. CITY remains INCOMPLETE.

**Current capture contract: `curb-decimal-v2`; LIVE ORDERED QUERY VERIFIED; LOSSLESS V2 SINGLE CAPTURE VERIFIED; LOSSLESS V2 DOUBLE CAPTURE VERIFIED; PRODUCTION PUBLICATION BLOCKED.** V2 preserves JSON decimal tokens without Number conversion, including every geometry/attribute numeric value. The [numeric contract](./CITY_CURB_SNAPSHOT_CONTRACT.md#11-lossless-numeric-v2-current-contract) and [double-capture evidence](./CITY_CURB_SNAPSHOT_CONTRACT.md#12-live-double-capture-validation-v2) supersede older precision/capture-blocker statements below. Two independent public DataSF captures, separated by a 15-second pause after A's reconstruction, each completed with 18,355 unique rows, 19 pages, zero retries and CONSISTENT state. All IDs, per-feature hashes, dataset/geometry digests and raw page bytes matched; metadata/schema/counts agreed and reconstruction passed for both. No database was contacted; the local SQL behavior results below still apply to unchanged V1 migration 00012.
...
**V2 precision boundary review:** the existing V1 storage design uses JSONB for geometry/raw source and numeric comparisons in its geometry gate. V2 uses a frozen `LosslessJsonNumber` with exact decimal digits/exponent, unquoted canonical numeric output, normalized equivalent values/negative zero and versioned `/v2` digest domains. It keeps the V1 codec and interval/local SQL fixtures pinned to V1. Source features reject native numbers; approximate spatial math requires the explicit diagnostic conversion and cannot feed authoritative hashes.

PostgreSQL JSONB can preserve numeric values within its numeric range, but capture bounds may exceed that range; ingestion must validate compatibility. The normal Supabase JS request/response serialization can lose precision or change numeric type. The recommended future interface accepts canonical JSON text, parses it to JSONB/numeric inside PostgreSQL, and returns authoritative values as text for lossless re-verification. No such interface is implemented here. Migration 00012's old canonicalization/manifest version and endpoint guards still reject V2 and need separately reviewed changes/tests. Full details and primary documentation are in the [snapshot contract](./CITY_CURB_SNAPSHOT_CONTRACT.md#future-database-and-client-precision-boundary-review-only). Do not put wrappers through normal JS object ingestion, weaken the V1 guard, or relabel old digests.

The numeric-contract and live double-capture blockers are resolved by 100 V2 and 34 preserved V1 offline checks, the prior single capture and two subsequent independent full captures with identical content. This does not establish transaction isolation or production readiness. V2 database/client compatibility and a precision-safe ingestion boundary, independent verifier authentication, source registration/rollout procedure and durable immutable artifact retention remain gates. No migration/schema/runtime/coverage change was made; CITY remains INCOMPLETE.
